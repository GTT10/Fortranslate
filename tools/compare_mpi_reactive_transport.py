...
def validate_reference(names: list[str], rows: list[list[float]]) -> None:
    required = {"x", "rho", "rhoE", "temperature", *SPECIES_COLUMNS}
    ...
    if not all(math.isfinite(value) for row in rows for value in row):
        raise AssertionError("MPI transport output contains non-finite values")
    if min(row[column["rho"]] for row in rows) <= 0.0:
        raise AssertionError("MPI transport output has non-positive density")
    if min(row[column["temperature"]] for row in rows) <= 0.0:
        raise AssertionError("MPI transport output has non-positive temperature")
    minimum_species = min(
        row[column[name]] for row in rows for name in SPECIES_COLUMNS
    )
    if minimum_species < -2.0e-12:
        raise AssertionError("MPI transport output violated species positivity")
    maximum_closure = max(
        abs(sum(row[column[name]] for name in SPECIES_COLUMNS) - row[column["rho"]])
        / row[column["rho"]]
        for row in rows
    )
    if maximum_closure > 2.0e-10:
        raise AssertionError("MPI transport output violated species closure")


if len(sys.argv) < 3:
    raise SystemExit(
        "usage: compare_mpi_reactive_transport.py reference.csv candidate.csv [...]"
    )
...
for filename in sys.argv[2:]:
    ...
    if candidate_names != names or len(candidate) != len(reference):
        raise AssertionError(f"{filename}: MPI transport output shape mismatch")
    maximum = 0.0
    ...
    if maximum > PARITY_TOLERANCE:
        raise AssertionError(f"{filename}: MPI transport rank-count parity failed")
