program test_reactive_directional_flux_3d
  ...
  real(dp), allocatable :: primitive(:), state(:), flux(:), reference_flux(:)
  ...
  allocate(reference_flux(reactive_nvar(size(species))))
  ...
  if (.not. ok) error stop "equal-state z HLLC flux failed"
  reference_flux = flux

  mass_flux = state(irho) * primitive(4)
  ...
  if (abs(flux(imy) - mass_flux * primitive(3)) > tolerance) &
    error stop "y-momentum placement mismatch"
  if (abs(flux(imz) - (mass_flux * primitive(4) + primitive(5))) > tolerance) &
    error stop "normal momentum placement mismatch"
  if (abs(flux(iet) - (state(iet) + primitive(5)) * primitive(4)) > tolerance) &
  ...
  call reactive_riemann_flux_z( &
    species, state, state, temperature, temperature, "rusanov", flux, ok)
  if (.not. ok) error stop "equal-state z Rusanov flux failed"
  if (maxval(abs(flux - reference_flux)) > tolerance) &
    error stop "Rusanov z equal-state inconsistency"

  call reactive_riemann_flux_z( &
    species, state, state, temperature, temperature, "pelec", flux, ok)
  if (.not. ok) error stop "equal-state z PeleC-style flux failed"
  if (maxval(abs(flux - reference_flux)) > tolerance) &
    error stop "PeleC-style z equal-state inconsistency"

  call reactive_riemann_flux_z( &
    species, state, state, temperature, temperature, "unknown", flux, ok)
  if (ok) error stop "unknown z solver accepted"

  write(*, '(a)') "test_reactive_directional_flux_3d: PASS"
end program test_reactive_directional_flux_3d
