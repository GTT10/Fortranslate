...
| `Source/Diffusion.*` multidimensional/AMR/EB responsibility | `reactive_transport_2d_mod` for regular 2D cells, `eb_reactive_transport_2d_mod` for one EB level, and `amr_reactive_1d_mod` for two-level 1D | Regular-grid 2D, single-level adiabatic-slip EB, and conservative two-level 1D AMR transport verified; EB AMR diffusive reflux remains |
