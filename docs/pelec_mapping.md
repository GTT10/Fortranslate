...
| `Source/EB.*` and AMReX-Hydro redistribution | `eb_geometry_2d_mod`, `eb_reactive_wall_flux_2d_mod`, `eb_reactive_redistribution_2d_mod` | Nodal geometry, reactive slip-wall pressure flux, open-face conservative divergence, first-order FluxRedist, and PeleC-default zeroth-order weighted StateRedist with EOS-validated transactional updates verified; higher-order StateRedist, periodic/ghost neighborhoods, EB flux generation, AMR, and MPI remain |
