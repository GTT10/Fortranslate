...
The `0.156.0` milestone contains the serial verification suite, eight optional
...
lifecycle, transport-enabled checkpoint/restart, arbitrary-depth EB
full-physics composition, and MPI distribution are not yet
...
subtree, and blends both Euler stages before commit. This core is not yet
connected to one combined `R-T-H-T-R` transaction, a public physics time loop,
or MPI ownership.
