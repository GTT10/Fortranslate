program test_entropy_wave
  ...
  real(dp) :: errors(number_of_resolutions)
  real(dp) :: order
  logical :: ok
  integer :: case_index
  ...
  do case_index = 1, number_of_resolutions
    ...
    if (.not. ok) error stop "Entropy-wave run failed"
    write(*, '(a,i0,a,es24.16)') "nx=", resolutions(case_index), ", L1=", &
      errors(case_index)
  end do

  do case_index = 1, number_of_resolutions - 1
    order = log(errors(case_index) / errors(case_index + 1)) / log(2.0_dp)
    write(*, '(a,i0,a,es24.16)') "refinement pair ", case_index, ", order=", order
    if (order < minimum_order) then
      write(*, '(a,es24.16,a,es24.16)') &
        "Observed order ", order, " is below ", minimum_order
      error stop 1
    end if
  end do

  write(*, '(a)') "test_entropy_wave: PASS"
...
contains
  ...
  subroutine run_entropy_wave(nx, density_l1_error, ok)
    ...
    conserved = 0.0_dp
    do i = 1, nx
      primitive(qrho) = 1.0_dp + density_amplitude * sin(2.0_dp * acos(-1.0_dp) * x(i))
    ...
    do while (time < final_time)
      call compute_cfl_timestep(conserved, nx, dx, gamma, cfl, dt, step_ok)
      if (.not. step_ok) then
        ok = .false.
        density_l1_error = huge(1.0_dp)
        return
      ...
      call advance_ssprk2( &
        ...
        reconstruction="plm", limiter="mc", boundary_condition="periodic")
      if (.not. step_ok) then
        ok = .false.
        density_l1_error = huge(1.0_dp)
        return
    ...
    ok = density_l1_error > 0.0_dp .and. density_l1_error < huge(1.0_dp)
  end subroutine run_entropy_wave

end program test_entropy_wave
