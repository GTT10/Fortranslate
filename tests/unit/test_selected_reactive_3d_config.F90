program test_selected_reactive_3d_config
  ...
  type(reactive_3d_config) :: config, valid_config
  ...
  if (command_argument_count() /= 1) then
    error stop "test_selected_reactive_3d_config requires an input path"
  end if
  ...
  if (trim(config%thermo_model) /= "selected") then
    error stop "selected reactive 3D thermo model was not read"
  end if
  valid_config = config

  call read_reactive_3d_configuration( &
    trim(input_path), config, ok, message)
  if (ok .or. index(message, "thermo_model") == 0) then
    error stop "ordinary reactive 3D parser accepted selected thermodynamics"
  end if
  ...
  if (abs(mole_fractions(1) - 0.8_dp) > 1.0e-14_dp .or. &
    ...
    error stop "selected reactive 3D composition was not mapped by name"
  end if
  ...
  if (ok .or. index(message, "duplicate species") == 0) then
    error stop "duplicate selected reactive 3D species was not rejected"
  end if
  ...
  if (ok .or. index(message, "not found exactly once") == 0) then
    error stop "unknown selected reactive 3D species was not rejected"
  end if
  ...
  if (ok .or. index(message, "nonfinite mole fraction") == 0) then
    error stop "nonfinite selected reactive 3D tail was not rejected"
  end if
  ...
  if (.not. ok .or. any(abs(mole_fractions - 0.5_dp) > 1.0e-14_dp)) then
    error stop "large selected reactive 3D composition was not scaled safely"
  end if

  write(*, '(a)') "test_selected_reactive_3d_config: PASS"
end program test_selected_reactive_3d_config
