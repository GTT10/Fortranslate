...
| `Source/PeleCAmr.*` hierarchy/synchronization subset | `amr_hierarchy_1d_mod`, `amr_regrid_1d_mod`, `amr_reactive_1d_mod` | Runnable reactive 1D two-level hierarchy with PCM/limited-PLM hydro, chemistry and molecular-transport splitting, hyperbolic/diffusive flux-consistent reflux, tagging, regridding, and composite output; characteristic PPM/multilevel pending |
