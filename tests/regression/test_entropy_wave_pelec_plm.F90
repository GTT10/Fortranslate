program test_entropy_wave_pelec_plm
  ...
  real(dp), parameter :: minimum_order = 1.7_dp
  ...
  do case_index = 1, number_of_resolutions
    ...
    if (.not. ok) error stop "PeleC-PLM entropy-wave run failed"
  ...
  write(*, '(a)') "test_entropy_wave_pelec_plm: PASS"
...
end program test_entropy_wave_pelec_plm
