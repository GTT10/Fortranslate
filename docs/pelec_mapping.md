...
| AMReX distributed-box responsibility | `mpi_domain_1d_mod`, `mpi_reactive_transport_1d_mod`, `mpi_reactive_1d_mod`, `mpi_amr_patch_1d_mod` | Uneven 1D blocks plus an owner-authoritative replicated AMR patch bridge, cross-rank sibling halos, and owner-only tree chemistry, recursive hydro, and recursive molecular transport with global rollback, verified for 1/2/4/8 ranks; combined patch-tree splitting and sparse AMR storage remain pending |
| `Source/PeleCAmr.*` hierarchy/synchronization subset | `amr_hierarchy_1d_mod`, `amr_multipatch_1d_mod`, `amr_patch_tree_1d_mod`, `amr_patch_tree_reactive_1d_mod`, `amr_regrid_1d_mod`, `amr_reactive_1d_mod`, `amr_multilevel_reactive_1d_mod`, `amr_multipatch_reactive_1d_mod`, `mpi_amr_patch_1d_mod` | Runnable tag-driven arbitrary-depth single-patch hierarchy, dynamic two-level multipatch application, automatically tagged arbitrary-depth branching reactive trees, adjacent sibling exchange, deterministic MPI patch ownership/halo communication, and owner-only chemistry/hydro/transport; combined splitting, sparse storage, and migration remain pending |
