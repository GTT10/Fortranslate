...
| `Source/Diffusion.*` multidimensional/AMR/EB responsibility | `reactive_transport_2d_mod` for regular 2D cells, `eb_reactive_transport_2d_mod` for EB levels, `amr_eb_transport_2d_mod` for two-level EB synchronization, and `amr_reactive_1d_mod` for two-level 1D | Regular-grid 2D, adiabatic-slip EB, conservative two-level EB AMR, and conservative two-level 1D AMR transport verified; deeper and sibling EB transport remain |
