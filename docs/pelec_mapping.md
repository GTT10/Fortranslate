...
| `Source/PeleCAmr.*` hierarchy/synchronization subset | `amr_hierarchy_1d_mod`, `amr_multipatch_1d_mod`, `amr_patch_tree_1d_mod`, `amr_patch_tree_reactive_1d_mod`, `amr_regrid_1d_mod`, `amr_reactive_1d_mod`, `amr_multilevel_reactive_1d_mod`, `amr_multipatch_reactive_1d_mod` | Runnable tag-driven arbitrary-depth single-patch hierarchy, dynamic two-level multipatch application, and static arbitrary-depth branching reactive PCM hydro with recursive hyperbolic/parabolic subcycling, reflux, elementary chemistry, and molecular transport; dynamic patch-tree rebuilds and MPI ownership remain pending |
