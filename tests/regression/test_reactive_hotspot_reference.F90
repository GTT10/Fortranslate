program test_reactive_hotspot_reference
  ...
  call compare_grid(32, "ppm", reference_state, reference_temperature, &
    ppm_error(1))
  ...
  if (ppm_error(2) >= 0.55_dp * ppm_error(1)) then
    error stop "Reactive PPM hotspot error did not decrease"
  end if
