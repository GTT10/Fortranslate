...
| `Source/Diffusion.*` multidimensional/AMR/EB responsibility | `reactive_transport_2d_mod` for regular 2D cells, `eb_reactive_transport_2d_mod` for EB levels and isothermal/no-slip wall fluxes, `amr_eb_transport_2d_mod`, `amr_eb_multilevel_transport_2d_mod`, and `amr_eb_multipatch_transport_2d_mod` for EB AMR synchronization, and `amr_reactive_1d_mod` for two-level 1D | Regular-grid 2D, adiabatic-slip or first-order isothermal/no-slip EB, conservative single-patch, three-level, separated sibling-patch and arbitrary-depth EB AMR, sparse-MPI ownership, and conservative two-level 1D AMR transport verified |
| AMReX distributed-box responsibility | `mpi_domain_1d_mod`, `mpi_reactive_transport_1d_mod`, `mpi_reactive_1d_mod`, `mpi_amr_patch_1d_mod`, `mpi_amr_sparse_patch_1d_mod`, `mpi_amr_eb_patch_2d_mod` | Uneven 1D blocks, subcycle-weighted 1D/EB AMR owner maps, rank-local 1D storage and sparse physics, direct 1D owner migration and traffic, plus 2D EB owner chemistry/hydro/transport/full-physics transactions, sparse numerical storage, targeted child-to-root average-down and hydro root traffic, direct owner-local fine SSPRK2 transport, and end-to-end sparse full-physics composition verified for 1/2/4/8 ranks |
...
| Sparse MPI EB AMR responsibility | PeleF 0.196.0 |
|---|---|
...
| public checkpoint compatibility | schema-5 structured mesh, EB wall, transport physics, StateRedist, prolongation method, hierarchy, regrid fingerprint, and cumulative transport-limiter diagnostic shared by serial and sparse-MPI applications with continuation and ownership controls explicitly excluded; fixed-depth formats store the same method in schema 3 |
