#!/usr/bin/env python3
"""Check public multipatch reactive EB AMR thermal conduction."""
...
def main() -> None:
    ...
    parser.add_argument("--reference-fine", required=True, nargs=2, type=Path)
    parser.add_argument("--transport-root", required=True, type=Path)
    parser.add_argument("--transport-fine", required=True, nargs=2, type=Path)
    args = parser.parse_args()

    reference_levels = [load(args.reference_root, 14 * 14)] + [
        load(path, 10 * 10) for path in args.reference_fine
    ]
    transport_levels = [load(args.transport_root, 14 * 14)] + [
        load(path, 10 * 10) for path in args.transport_fine
    ]
    reference_hierarchy_temperature: list[float] = []
    transport_hierarchy_temperature: list[float] = []
    maximum_change = 0.0
    for level, (reference, transported) in enumerate(
        zip(reference_levels, transport_levels)
    ):
        if [row["cell_type"] for row in reference] != [
            row["cell_type"] for row in transported
        ]:
            raise AssertionError(f"level {level}: EB classification changed")
    ...
    if reference_span - transport_span <= 1.0e-8:
        raise AssertionError(
            "multipatch conduction did not reduce the hierarchy temperature span"
        )
    if maximum_change <= 1.0e-8:
        raise AssertionError("multipatch thermal transport produced no change")
    print("check_reactive_eb_amr_multipatch_transport_2d: PASS")
