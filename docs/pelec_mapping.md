...
| `Source/EB.*` and AMReX-Hydro redistribution | `eb_geometry_2d_mod`, `eb_reactive_wall_flux_2d_mod`, `eb_reactive_redistribution_2d_mod`, `eb_reactive_hydro_2d_mod` | Nodal geometry, reactive slip-wall pressure flux, piecewise-constant Riemann fluxes on open faces, complete first-order EB hydro update, FluxRedist, and PeleC-default zeroth-order weighted StateRedist with EOS-validated transactions verified; higher-order EB reconstruction/StateRedist, periodic/ghost neighborhoods, AMR, and MPI remain |
