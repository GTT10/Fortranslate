...
| `Source/PeleCAmr.*` hierarchy/synchronization subset | `amr_hierarchy_1d_mod`, `amr_multipatch_1d_mod`, `amr_regrid_1d_mod`, `amr_reactive_1d_mod`, `amr_multilevel_reactive_1d_mod`, `amr_multipatch_reactive_1d_mod` | Runnable tag-driven arbitrary-depth single-patch hierarchy plus fixed two-level separated multipatch WENO hydro, chemistry, and transport; conservative multipatch planning, transfer, reflux, and composite integration are qualified, while dynamic multipatch depth and MPI ownership remain pending |
