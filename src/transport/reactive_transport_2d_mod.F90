...
contains
  ...
  subroutine reactive_transport_fluxes_2d( &
    ...
    real(dp) :: dudx, dvdx, dwdx, dudy, dvdy, dwdy, divu
    real(dp) :: tau_xx, tau_xy, tau_xz, tau_yx, tau_yy, tau_yz
    real(dp) :: dtdx, dtdy, energy_species, outgoing, mass, candidate
    real(dp) :: theta_face, uface, vface, wface
    logical :: local_ok
    integer :: i, j, ip, im, jp, jm, k, component, nspecies, nvar, nprim
    ...
    if (size(transport) /= nspecies .or. nx < 2 .or. ny < 2 .or. &
        ...
        size(flux_x, 2) < nx .or. size(flux_x, 3) < ny .or. &
        size(flux_y, 1) /= nvar .or. size(flux_y, 2) < nx .or. &
        size(flux_y, 3) < ny) return
    ...
    allocate(species_energy_x(nx, ny), species_energy_y(nx, ny))
    ...
    do j = 1, ny
      ...
      do i = 1, nx
        ...
        call face_transport_data( &
          species, transport, primitive(:, i, j), primitive(:, ip, j), &
          checked_temperature(i, j), checked_temperature(ip, j), viscosity, &
          conductivity, diffusion, yleft, yright, yface, xleft, xright, &
          hface, local_ok)
        if (.not. local_ok) return
        density_face = 0.5_dp * (primitive(1, i, j) + primitive(1, ip, j))
        pressure_face = 0.5_dp * (primitive(5, i, j) + primitive(5, ip, j))
        dudx = (primitive(2, ip, j) - primitive(2, i, j)) / dx
        dvdx = (primitive(3, ip, j) - primitive(3, i, j)) / dx
        dwdx = (primitive(4, ip, j) - primitive(4, i, j)) / dx
        dudy = 0.25_dp * ( &
          primitive(2, i, jp) - primitive(2, i, jm) + &
          primitive(2, ip, jp) - primitive(2, ip, jm)) / dy
        dvdy = 0.25_dp * ( &
          primitive(3, i, jp) - primitive(3, i, jm) + &
          primitive(3, ip, jp) - primitive(3, ip, jm)) / dy
        divu = dudx + dvdy
        tau_xx = 0.0_dp
        tau_xy = 0.0_dp
        tau_xz = 0.0_dp
        if (viscosity_enabled) then
          ...
          flux_x(imx, i, j) = -tau_xx
          flux_x(imy, i, j) = -tau_xy
          flux_x(imz, i, j) = -tau_xz
          uface = 0.5_dp * (primitive(2, i, j) + primitive(2, ip, j))
          vface = 0.5_dp * (primitive(3, i, j) + primitive(3, ip, j))
          wface = 0.5_dp * (primitive(4, i, j) + primitive(4, ip, j))
          flux_x(iet, i, j) = -(tau_xx * uface + tau_xy * vface + &
            tau_xz * wface)
        end if
        if (thermal_conduction_enabled) then
          dtdx = (checked_temperature(ip, j) - checked_temperature(i, j)) / dx
          flux_x(iet, i, j) = flux_x(iet, i, j) - conductivity * dtdx
        end if
        if (species_diffusion_enabled) then
          call species_face_flux( &
            species, diffusion, yleft, yright, yface, xleft, xright, hface, &
            density_face, primitive(5, i, j), primitive(5, ip, j), &
            pressure_face, dx, barodiffusion_enabled, species_flux, &
            energy_species, local_ok)
          if (.not. local_ok) return
          do k = 1, nspecies
            flux_x(reactive_species_component(k), i, j) = species_flux(k)
          end do
          species_energy_x(i, j) = energy_species
          flux_x(iet, i, j) = flux_x(iet, i, j) + energy_species
        end if
        flux_x(irho, i, j) = 0.0_dp

        ! y-normal face between (i,j) and (i,j+1).
        call face_transport_data( &
          species, transport, primitive(:, i, j), primitive(:, i, jp), &
          checked_temperature(i, j), checked_temperature(i, jp), viscosity, &
          conductivity, diffusion, yleft, yright, yface, xleft, xright, &
          hface, local_ok)
        if (.not. local_ok) return
        density_face = 0.5_dp * (primitive(1, i, j) + primitive(1, i, jp))
        pressure_face = 0.5_dp * (primitive(5, i, j) + primitive(5, i, jp))
        dudy = (primitive(2, i, jp) - primitive(2, i, j)) / dy
        dvdy = (primitive(3, i, jp) - primitive(3, i, j)) / dy
        dwdy = (primitive(4, i, jp) - primitive(4, i, j)) / dy
        dudx = 0.25_dp * ( &
          primitive(2, ip, j) - primitive(2, im, j) + &
          primitive(2, ip, jp) - primitive(2, im, jp)) / dx
        dvdx = 0.25_dp * ( &
          primitive(3, ip, j) - primitive(3, im, j) + &
          primitive(3, ip, jp) - primitive(3, im, jp)) / dx
        divu = dudx + dvdy
        tau_yx = 0.0_dp
        tau_yy = 0.0_dp
        tau_yz = 0.0_dp
        if (viscosity_enabled) then
          ...
          flux_y(imx, i, j) = -tau_yx
          flux_y(imy, i, j) = -tau_yy
          flux_y(imz, i, j) = -tau_yz
          uface = 0.5_dp * (primitive(2, i, j) + primitive(2, i, jp))
          vface = 0.5_dp * (primitive(3, i, j) + primitive(3, i, jp))
          wface = 0.5_dp * (primitive(4, i, j) + primitive(4, i, jp))
          flux_y(iet, i, j) = -(tau_yx * uface + tau_yy * vface + &
            tau_yz * wface)
        end if
        if (thermal_conduction_enabled) then
          dtdy = (checked_temperature(i, jp) - checked_temperature(i, j)) / dy
          flux_y(iet, i, j) = flux_y(iet, i, j) - conductivity * dtdy
        end if
        if (species_diffusion_enabled) then
          call species_face_flux( &
            species, diffusion, yleft, yright, yface, xleft, xright, hface, &
            density_face, primitive(5, i, j), primitive(5, i, jp), &
            pressure_face, dy, barodiffusion_enabled, species_flux, &
            energy_species, local_ok)
          if (.not. local_ok) return
          do k = 1, nspecies
            flux_y(reactive_species_component(k), i, j) = species_flux(k)
          end do
          species_energy_y(i, j) = energy_species
          flux_y(iet, i, j) = flux_y(iet, i, j) + energy_species
        end if
        flux_y(irho, i, j) = 0.0_dp
      end do
    end do

    ! A single limiter coefficient is shared by all species on each face.  This
    ! preserves sum(j_k)=0 and keeps the matching species-enthalpy flux scaled
    ! by the same amount.
    if (species_diffusion_enabled .and. dt > 0.0_dp) then
        ...
        do i = 1, nx
          ...
          do k = 1, nspecies
            ...
            outgoing = max(flux_x(component, i, j), 0.0_dp) / dx + &
              max(-flux_x(component, im, j), 0.0_dp) / dx + &
              max(flux_y(component, i, j), 0.0_dp) / dy + &
              max(-flux_y(component, i, jm), 0.0_dp) / dy
      ...
      do j = 1, ny
        ...
        do i = 1, nx
          ip = periodic_index(i + 1, nx)
          theta_face = min(theta_cell(i, j), theta_cell(ip, j))
          minimum_theta = min(minimum_theta, theta_face)
          do k = 1, nspecies
            component = reactive_species_component(k)
            flux_x(component, i, j) = theta_face * flux_x(component, i, j)
          end do
          flux_x(iet, i, j) = flux_x(iet, i, j) + &
            (theta_face - 1.0_dp) * species_energy_x(i, j)

          theta_face = min(theta_cell(i, j), theta_cell(i, jp))
          minimum_theta = min(minimum_theta, theta_face)
          do k = 1, nspecies
            component = reactive_species_component(k)
            flux_y(component, i, j) = theta_face * flux_y(component, i, j)
          end do
          flux_y(iet, i, j) = flux_y(iet, i, j) + &
            (theta_face - 1.0_dp) * species_energy_y(i, j)
        end do
      end do
    end if
  ...
  subroutine reactive_transport_euler_update_2d( &
    ...
    call reactive_transport_fluxes_2d( &
      species, transport, input_state, input_temperature, nx, ny, dx, dy, dt, &
      viscosity_enabled, thermal_conduction_enabled, species_diffusion_enabled, &
      barodiffusion_enabled, flux_x, flux_y, minimum_theta, local_ok)
    if (.not. local_ok) return
    ...
    do j = 1, ny
      ...
      do i = 1, nx
        ...
        output_state(:, i, j) = input_state(:, i, j) - &
          dt / dx * (flux_x(:, i, j) - flux_x(:, im, j)) - &
          dt / dy * (flux_y(:, i, j) - flux_y(:, i, jm))
        call reactive_conserved_to_primitive( &
  ...
  subroutine advance_reactive_transport_2d( &
    ...
    call reactive_transport_euler_update_2d( &
      species, transport, initial_state, initial_temperature, nx, ny, dx, dy, &
