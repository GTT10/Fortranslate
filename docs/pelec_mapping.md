...
| `Source/PeleCAmr.*` hierarchy/synchronization subset | `amr_hierarchy_1d_mod`, `amr_regrid_1d_mod`, `amr_reactive_1d_mod`, `amr_multilevel_reactive_1d_mod` | Runnable tag-driven arbitrary-depth nested hierarchy with recursive PCM/limited-PLM reactive hydro, chemistry, molecular transport, conservative periodic hierarchy rebuilds, and composite output; multiple patches, fine-overlap retention, MPI ownership, and characteristic PPM pending |
