program test_reactive_characteristic_ppm_convergence
  ...
  if (.not. ok) error stop "Failed to load characteristic PPM thermodynamics"
  call load_h2o2_elementary_mechanism(reactions, ok)
  if (.not. ok) error stop "Failed to load characteristic PPM mechanism"
  ...
  write(*, '(a,3(1x,es16.8))') "characteristic PPM entropy-wave density L1:", entropy_error
  write(*, '(a,2(1x,f10.6))') "characteristic PPM entropy-wave orders:", entropy_order
  write(*, '(a,3(1x,es16.8))') &
    "characteristic PPM composition-wave H2 L1:", composition_error
  write(*, '(a,2(1x,f10.6))') &
    "characteristic PPM composition-wave orders:", composition_order

  if (minval(entropy_order) < 2.05_dp .or. entropy_error(3) > 1.2e-5_dp) then
    error stop "Reactive characteristic PPM entropy-wave convergence failed"
  end if
  if (minval(composition_order) < 2.00_dp .or. &
      composition_error(3) > 1.8e-6_dp) then
    error stop "Reactive characteristic PPM composition-wave convergence failed"
  end if
  write(*, '(a)') "test_reactive_characteristic_ppm_convergence: PASS"
...
contains
  ...
  subroutine run_entropy(nx, l1_error)
    ...
    if (.not. run_ok) error stop "Reactive characteristic PPM entropy-wave run failed"
  ...
  subroutine run_composition(nx, l1_error)
    ...
    call simulate_reactive_1d( &
      species, reactions, config, state, temperature, dx, time, steps, &
      initial_integrals, final_integrals, run_ok)
    if (.not. run_ok) error stop "Reactive characteristic PPM composition-wave run failed"
    ...
    do cell = 1, nx
      ...
      if (.not. run_ok) error stop "Invalid characteristic PPM composition-wave state"
  ...
  subroutine require_conservation(initial_integrals, final_integrals)
    ...
    if (error > 5.0e-12_dp) then
      error stop "Reactive characteristic PPM conservation failed"
    end if
  end subroutine require_conservation

end program test_reactive_characteristic_ppm_convergence
