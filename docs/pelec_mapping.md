...
| `Source/PeleCAmr.*` hierarchy/synchronization subset | `amr_hierarchy_1d_mod`, `amr_regrid_1d_mod`, `amr_reactive_1d_mod`, `amr_multilevel_reactive_1d_mod` | Runnable tag-driven arbitrary-depth nested hierarchy with recursive PCM/limited-PLM/characteristic-PPM and hybrid-WENO reactive hydro, chemistry, molecular transport, conservative periodic rebuilds, aligned fine-overlap retention, outflow-boundary refinement, one-sided reflux, and composite output; multiple patches and MPI ownership remain pending |
