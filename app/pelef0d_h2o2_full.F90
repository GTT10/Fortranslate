program pelef0d_h2o2_full
  ...
  use pressure_dependent_kinetics_mod, only: &
    pressure_dependent_reaction, pressure_dependent_production_rates
  ...
  integer :: argument_count, unit, io_status, steps, total_newton, total_rejected
  ...
  do while (time < config%final_time - time_tolerance)
    ...
    call advance_implicit_adaptive( &
      ...
      config%minimum_time_step, config%maximum_time_step, &
      config%relative_tolerance, config%absolute_tolerance, &
      config%maximum_newton_iterations, mass_fractions, temperature, &
      accepted_step, suggested_step, newton_iterations, rejected_steps, ok)
    if (.not. ok) error stop "Implicit full H2/O2 step failed"
    time = time + accepted_step
    requested_step = suggested_step
    steps = steps + 1
