...
| `Source/EB.*` and AMReX-Hydro redistribution | `eb_geometry_2d_mod`, `eb_reactive_wall_flux_2d_mod`, `eb_reactive_redistribution_2d_mod`, `eb_reactive_hydro_2d_mod`, `reactive_eb_2d_driver_mod`, `pelef_reactive_eb_2d` | Nodal geometry, reactive slip-wall pressure flux, piecewise-constant Riemann fluxes on open faces, complete first-order EB hydro update, FluxRedist, PeleC-default zeroth-order weighted StateRedist, active-cell chemistry splitting, and an input-driven serial time loop with EOS-validated transactions verified; higher-order EB reconstruction/StateRedist, periodic/ghost neighborhoods, molecular transport, AMR, and MPI remain |
