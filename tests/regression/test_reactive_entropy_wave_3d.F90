program test_reactive_entropy_wave_3d
  ...
  use reactive_3d_mod, only: &
    ...
    advance_reactive_euler_ssprk2_3d, reactive_integrals_3d, &
    reactive_extrema_3d
  implicit none

  integer, parameter :: case_count = 3
  integer, parameter :: resolutions(case_count) = [6, 12, 24]
  real(dp), parameter :: minimum_order = 0.70_dp
  ...
  do case_index = 1, case_count
    ...
    if (.not. ok) error stop "Reactive 3D entropy-wave run failed"
    ...
    if (conservation_errors(case_index) > conservation_tolerance) then
      error stop "Reactive 3D conservation error exceeds threshold"
    end if
    if (closure_errors(case_index) > closure_tolerance) then
      error stop "Reactive 3D species closure exceeds threshold"
    end if
  end do
  ...
  do case_index = 1, case_count - 1
    ...
    if (order < minimum_order) then
      error stop "Reactive 3D entropy-wave order is below threshold"
    end if
  end do
  write(*, '(a)') "test_reactive_entropy_wave_3d: PASS"
...
end program test_reactive_entropy_wave_3d
